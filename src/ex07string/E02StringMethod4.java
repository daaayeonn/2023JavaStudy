...
public class E02StringMethod4 {
	...
	public static void main(String[] args) {
		...
		/*
		시나리오 1] 주민등록번호를 이용하여 성별을 판단하는
		프로그램을 charAt()을 이용해서 작성하시오.
		190419-3000000 => 남자
		190419-4000000 => 여자
		*/
		
//		String str1 = "190419-4000000";
//		System.out.println(str1.charAt(7) == '3' ? "남자" : "여자");
		...
		/*
		 ...
		 이를 통해 판단한다. 추출했을 때는 char형이므호 ''(싱글쿼테이션)으로 비교해야 한다.
		 */
		...
		*/
		...
		/*
		 이메일은 아이디와 도메인을 @(앳)으로 구분한다. 또한 도메인에는 하나 이상의 .(딧)이
		 포함된다. 따라서 두 가지 조건 모두 만족해야 하므로 논리And를 사용해서 판단한다.
		 */
		...
		/*
		시나리오3]주민등록번호로 성별을 구분하는 프로그램을 indexOf()를
		이용해서 작성하시오.
		*/
		System.out.println("### 시나리오 3 ###");
		String jumin  = "190419-2000000";
		/* 주민번호 문자열에서 하이픈의 위치를 찾는다. 성별은 그 다음에 있으므로 1을 더해
		 인덱스를 결정한다.*/
		int index = jumin.indexOf("-") + 1;
		// 앞에서 구한 인덱스를 charAt()의 인수로 전달한다.
		if (jumin.charAt(index) == '1' || 
				jumin.charAt(index) == '3') {
			System.out.println("남자입니다.");
		}
		else if (jumin.charAt(index) == '2' || 
					jumin.charAt(index) == '4') {
			System.out.println("여자입니다.");
		}
		else {
			System.out.println("주민번호가 잘못되었습니다.");
...
//		System.out.println(file.substring(14));
		
		System.out.println("방법 1 - lastIndexOf() 사용");
		...
		System.out.println("방법 2 - split() 사용");
		/*
		 Java에서 split() 메서드 사용 시 구분자로 .(닷)을 사용하는 경우네는 []를
		 씌워줘야 한다.
		 */
