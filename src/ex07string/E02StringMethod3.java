...
public static void main(String[] args) {
		...
		/*
		11] lastIndexOf() : 사용법은 indexOf()와 동일하지만 문자열의 뒤에서부터
			찾는 것이 다른 점이다.
		 */
		...
		/*
		12] replace : 특정 문자열을 찾아서 지정된 문자열로 변경한다. 민약 찾는 문자열이
			없다면 되지않을뿐 에러가 발생하진 않는다.
		 */
		...
		13] split() : 문자열을 구분자를 통해 분리해서 String타입의 배열로 반환한다.
			해당 구분자가 없는 경우네는 크기가 1인 배열로 반환한다.
		 */
		...
		/*
		 구분자로 사용한 문자열을 없는 경우 문자열 전체가 통째로 반환되어 배열의 크기는
		 1이 된다.
		 */
		...
		방법 1: 인덱스를 하나만 사용 => 이경우 인덱스부터 문자열의 끝까지를 잘라낸다.
		방법 2: 시작인덱스, 종료인덱스를 사용 => 인덱스에 해당하는 부분만 잘라서 반환한다.
		 */
