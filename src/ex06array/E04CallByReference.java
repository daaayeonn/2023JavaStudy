...
/*
 ...
 : 메서드를 호출할 때 참고밧을 전달함으로써 서로 다른 지역에서도 동일한 메모리(Heap영역)을
 참조할 수 있게하는 호출 방식이다.
 메서드의 지역이 다르더라도 참조값을 통해 인스턴스를 참조하게되므로 A지역에서의 값의 변경이
 B지역에서도 그대로 참도되는 장점이 있다.
...
public class E04CallByReference {
	...
	public static void main(String[] args) {
		...
		/*
		 지역은 달라도 참조값을 통해 동일한 메모리를 참도하게되므로 동일한 출력결과가 나온다.
		 출력3 : 200, 100
		 */
	...
	static void callByReference(int[] ref) {
		/*
		 매개변수를 통해 전달받은 배열 인스턴스의 참조값을 통해 0번 원소와 1번 원소를 서로
		 교환한다. 이 때 배열에 저장된 값이 서로 바뀌게된다.
		 */
		int temp;
		temp = ref[0];
		ref[0] = ref[1];
