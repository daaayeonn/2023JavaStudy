...
/*
 foreach문 (개선된 for문)
 : 배열의 일부가 아니라 전체를 참조해야할 때 주로 사용하는 for문을 말한다.
...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		/*
		 배열 arr의 원소를 순서대로 하나씩 참조하여 출력하고, 더이상 참조 할 원소가 없을 때
		 for문이 자동으로 종료된다.
		 */
		sum = 0;
		for(int e : arr) {
		...
		/*
		 1 증가되는 부분이 기존 배열에 적용되지 않으므로 초기화했던 상태 그대로를 유지한다.
		 */
		for(int c : arr) {
			System.out.print(c + " ");
		}
		...
		System.out.println("2차원 배열을 foreach 문으로 출력하기");
		int[][] twoDim = {
				{1, 2, 3, 4},
				{10, 20, 30, 40},
				{100, 200, 300, 400}
