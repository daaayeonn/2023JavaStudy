...
/*
 equals() 메서드
 - 자바에서 인스턴스의 내용 비교를 위해 비교연산자(==)를 사용하는 것은 단순히 참조값에 대한 비교를
 	하는 것이므로, 실제 내용에 대한 비교가 이뤄지지 않는다.
 - 인스턴스간의 내용 비교를 위해서는 equals() 메서드를 각 클래스에 맞게 오버라이딩하여 사용해야
 	한다. 
 - String과 같은 기본 클래스는 별도의 오버라이딩 없이도 내용 비교가 가능하다.
 */

// Wrapper 클래스 중 Integer 와 유사한 형태로 클래스 제작
class IntNumber {
	// 멤버변수와 생성자
	int num;
	...
	/*
	 equals() 메서드가 어떻게 인스턴스 간의 내용을 비교하는지 설명하기 위한 코드로 작성함.
	 */
	// 비교를 위해 IntNumber 타입의 인스턴스를 매개변수로 받은 후..
	public boolean myEquals(IntNumber numObj) {
		// 매개변수로 받은 인스턴스의 멤버변수와 값을 비교한다.
...
public class E04Object2MyEquals {
	...
	public static void main(String[] args) {
		...
		/*
		 개발자가 직접 정의한 myEquals() 매서드를 통해 객체간의 내용비교를 한다.
		 여기서는 다르다는 결과가 출력된다.
		 */
		...
		/*
		 인스턴스간의 비교를 할 때는 단순히 참조변수에 대한 비교를 하면 안 되고 실제 내용에 대한
		 비교를 해야한다.
		 */
