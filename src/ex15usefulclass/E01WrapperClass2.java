...
//				Integer.parseInt(money)); // 예외발생
		
		String floatString = "3.14";
		/*
		 실수형태의 문자열을 숫자로 변경할 때는 parseInt()를 사용할 수 없다.
		 앞과 동일한 예외가 발생한다.
		 */
//		System.out.println(Integer.parseInt(floatString)); // 예외발생
		...
		/*
		 ...
		 codePointAt() : 문자열에서 특정 index에 해당하는 문 자의 아스키코드를 반환한다.
		 */
		System.out.println("ABCD에서 3번째 인덱스 D의 아스키코드");
		System.out.println(Character.codePointAt("ABCD", 3));
		
		/*
		 isDigit() : 인수로 전달된 문자가 숫자인지 판단한다. 숫자인 경우 true를 반환한다.
		 */
		...
		/*
		 isLetter() : 문자인지 판단하는 메서드로 특수기호나 숫자형일떄 false를 반환한다.
		 */
		...
		/*
		 isWhitespace() : 공백문자인지 판단하는 메서드. 공백문자일때 true를 반환한다.
		 */
		...
		/*
		 시나리오] 주어진 문자열 안에 몇 개의 공백문자(스페이스)가 있는지 판단하는 프로그램을 
		 작성하시오.
		 	참고) toCharArray() : 문자열을 char형 배열로 반환해주는 String클래스에 
		 	정의된 메소드
		 */
	}
}