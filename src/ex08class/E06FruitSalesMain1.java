...
class FruitSeller{
	...
	/*
	 멤버변수의 경우 초기값이 없는 상태로 정의한 후 인스턴스를 생성하고 차후 초기화 할 수 있다.
	 하지만 멤버상수는 인스턴스 생성과 상관없이 정의하는 시점에 반드시 초기화 해야한다.
	...
	/*
	 ...
	 1. 구매자로부터 금액을 받는다. (메서드 호출 시 매개변수를 통해 받는다)
...
class FruitBuyer{
	...
	/*
	 ...
	 1. 판매자에게 금액을 지불한다. => 여기에서 seller 인스턴스의 saleAplle()메서드가
	 호출된다.
...
public class E06FruitSalesMain1 {
	...
	public static void main(String[] args) {
		...
		/* 핀메자와 구매자의 인스턴스를 생성한다. 여기서는 우리가 직접 생성자를 만들지 
		 않았으므로 자동으로 생성되는 디폴트 생성자를 통해 인스턴스를 생성하게된다.
		 */
