...
/*
 ...
 : 메인쓰레드의 Working쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로 메인쓰레드가 종료되어도
 계속 실행되며, Dead상태가 되어야만 실행이 종료되는 쓰레드를 말한다. 해당 쓰레드는 main()
 메서드가 종료된 이후에도 계속 실행되는 것을 볼 수 있다.
 */
class NormalThread extends Thread {
	...
	/*
	 Thread 클래스의 생성자 호출 시 전달하는 인수를 통해 해당 쓰레드의 이름을 지정할 수 
	 있다. 이 때 지정된 이름은 getName()을 통해 반환받을 수 있다.
	 */
	...
	/*
	 쓰레드 동작중 비정상적으로 종료될 때 해당 예외가 발생할 수 있으므로 예외처리를 하거나
	 아래와 같이 예외던지기를 해서 호출한 위치로 예외 객체를 던질수도 있다.
	 */
	void threadMethod() throws InterruptedException {
		/*
		 10번 반복 중 2초씩 블럭상태가 되므로 총 20초간 실행하면서 반복횟수와 쓰레드명을
		 출력한다.
		 */
		for(int i = 1; i <= 10; i++) {
...
/*
 ...
 메인메서드도 하나의 독립쓰레드 이므로, 다른 독립쓰레드가 남아있다면 종료될때까지 프로그램이 종료된
 것이 아니다.
 주로 프로그램의 보조역할로 배경음악재생, 로그자동저장 등의 업무를 처리한다. 프로그램이 종료될 때
 함께 종료되므로 무한루프로 구현한다.
 */
	...
	public void run() {
		while(true) {
			...
			try {
				...
				System.out.println("자동저장 시 오류발생 ㅜㅜ");
			}
		}
	}
}
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/*
		 쓰레드로 구현하지 않는 일반적인 클래스의 작업으로 순차적으로 실행되어 1번 인스턴스가
		 작업을 마쳐야만 2번째가 실행된다.
		 */
		...
		/*
		 첫번째 독립쓰레드 생성 및 시작, 우선순위를 별도로 부여하지 않으므로 5로 지정된다.
		 쓰레드 인스턴스 생성 시 이름을 부여하는 방법으로 아래 2가지가 있다.
		 */
...
//		yt1.setName("First 쓰레드");
		...
		/*
		 두번째 독립쓰레드 생성 및 시작. 쓰레드명을 별도로 부여하지 않으면 JVM이 자동으로
		 부여한다.
		 */
		...
		/*
		 데몬쓰레드는 독립쓰레드를 먼저 생성한 후 setDaemon()을 통해 만들 수 있다.
		 */
		DaemonThread dt = new DaemonThread();
		dt.setName("데몬쓰레드"); // 이름지정
		...
		System.out.println("메인 함수의 우선 순위: " + 
