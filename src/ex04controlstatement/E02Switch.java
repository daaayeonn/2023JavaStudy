...
/*
 switch문
 : if문처럼 조건에 따라 분기하는 제어문으로, 정수식의 값과 동일한 부분을 찾아 실행하는 형태를
 가지고 있다.
 형식]
 	switch(산술식 혹은 정수식) {
 	case 값1:
 	...
 	※ switch문 내부에서 break문을 만나게되면 실행이 중지되고 밖으로 탈출하게 된다.
...
public class E02Switch {
	...
	public static void main(String[] args) {
		...
		/*
		 Scanner클래스
		 : 사용자로부터 입력값을 받기 위한 클래스로, 해당 클래스의 메서드가 실행되면
		 잠깐 실행이 중지되고 입력을 기다린다.
		 nextLine() : 문자열을 입력받는다.
		 nextInt() : 정수를 입력받는다.
		 */
		...
		default:
			...
			//default 다음에는 실행할 문장이 없으므로 break를 쓰지않는다.
		}
...
//		}
		...
		/*
		 ...
		 if문에서 ||(논리Or)을 사용하는 것과 동일하다.
		 */
		int season = 8;
		switch(season) {
		case 3:case 4: case 5:
		//=> if(season==3 || season==4 || ...)
		//if문으로 수정하면 위와같이 작성할 수 있다.
