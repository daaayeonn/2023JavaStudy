...
/*
 해당 클래스는 sub 패키지에 정의되었으므로 사용을 위해서는 반드시 import 후 사용해야한다.
 */
...
//		one.privateMethod(); // 에러발생
		...
		/*
		 다른 패키지에 선언된 클래스이므로 인스턴스 생성을 위해 import 선언을 해야한다. 또한
		 패키지가 다르면 privatem default 멤버는 접근할 수 없다.
		 */
...
//		two.defaultMethod(); // 에러발생
		...
		/*
		 다른 패키지에 선언된 디폴트 클래스이므로 인스턴스 생성이 불가능하다. 또한 import 자체도
		 불가능하므로 아예 사용할 수 없는 클래스가 된다.
		 */
		System.out.println("DefaultClass 객체 생성 및 접근");
