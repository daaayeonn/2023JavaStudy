...
/*
 ...
 - 추상클래스는 대부분 추상메서드를 포함하고 있으므로, 이를 상속받는 클래스는 반드시 추상메서드를
 	오버라이딩 해야한다.
...
/*
 추상메서드를 포함하지 않은 추상클래스를 상속하면 오버라이딩은 강제사항이 아닌 선택 사항이다.
 즉 필요한 경우에만 오버라이딩 하면 된다.
...
/*
 위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속하게 되므로 해당 클래스도 
 abstract로 선언해야한다. 단, 이경우 해당 클래스로도 인스턴스를 생성할 수 없게된다.
 따라서 인스턴스를 생성하기 위해서는 부모의 추상메서드를 자식쪽에서 오버라이딩 해야한다.
 */
class AbsClassChild2 extends AbsClass2 {
	/*
	 오버라이딩은 부모쪽의 메서드를 재정의하는 것이므로 해당 클래스는 추상메서드를 포함하지 않은
	  상태가 된다. 따라서 인스턴스 생성이 가능해진다.
	 아래 오버라이딩 한 메서드를 삭제하면 에러가 발생한다.
	 */
	@Override
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		// 추상클래스는 인스턴스 생성은 안 되지만, 참조변수로 사용할 수 있다.
		...
		/*
		 추상클래스내에서 선언된 정적메서드는 오버라이딩의 대상이 될 수 없으므로 static의 
		 기본규칙을 따른다. 별도의 생성절차 없이 클래스명으로 바로 접근할 수 있다.
		 */
