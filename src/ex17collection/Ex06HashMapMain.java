...
/*
 ...
 - Key는 중복될 수 없다. 만약 중복되면 기존의 데이터를 덮어쓰기 한다.
...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		 Map컬렉션 생성. Key의 Value를 모두 String으로 선언함.
		 */
		HashMap<String, String> map = new HashMap<String, String>();
		
		/*
		 인스턴스저장 : 최초 저장이라면 성공 시 null을 반환한다.
		 기존에 이미 저장된 동일한 Key가 존재하면 저장된 인스턴스가 반환된다.
		 */
		System.out.println("name이라는 키값으로 저장된 이전의 값: " + 
				map.put("name", "홍길동"));
		
		int number = 20;
		// int형은 String으로 반환할 수 없으므로 타입에러 발생됨
//		map.put("age", number);
		...
		/*
		 중복저장 : 기존에 입력된 name이라는 Key가 있으므로 "홍길동"이 반환된다. 그리고
		 기존의 값은 "최길동"으로 수정된다.
		 */
		...
		/*
		 출력하기
		 	1. 키 값을 알고있을 때 : get(key)으로 출력한다.
		 */
		System.out.println("키 값을 알 때: " + map.get("name"));
		
		/*
		 2. 키 값을 모를 때(혹은 너무 많아서 하나씩 명시하기 힘들 때)
		 : keySet()을 통해 전체 key를 얻어온다. 이를 통해 for문으로 순차적인 접근을
		 하면된다.	
		 */
		...
		/*
		 3. 이터레이터를 통한 출력
		 : 이터레이터를 통해 출력할 때도 ketSet()을 통해 Key를 얻어온 후 반복출력한다.
		 */
		...
		/*
		 Value만 얻어올때는 values()를 통해 값을 인출할 수 있다.
		 반환되는 값은 collectiob<T> 타입이다.
		 */
		...
		/*
		 인스턴스 존재유무확인
		 : Map컬렉션은 Key와 Value가 있으므로 메서드도 아애와 같이 2개로 나눠져있다.
		 */
		...
		/*
		 전체 삭제 : removeAll()은 없고, Clear()만 사용할 수 있다.
		 */
		map.clear();
