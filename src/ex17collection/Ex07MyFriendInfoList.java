...
public class Ex07MyFriendInfoList {
	...
	public static void main(String[] args) {
		...
		/* 컬렉션은 최고 생성 시 크기를 지정하지 않는다. 입력 혹은 삭제되는 인스턴스에
		 * 따라 크기가 자동으로 설정된다. */
...
class FriendInfoHandler {
	// 멤버변수 : [기존]인스턴스 배열 => [변경]List 컬렉션
	/*
	 기존에는 카운트를 위한 별도의 변수가 필요했지만, 컬렉션은 자동인덱싱을 지원하므로
	 필요하지 않다. 즉, 컬렉션 인스턴스만 있으면 된다.
	 */
	ArrayList<Friend> lists;
	
	/*
	 멤버변수가 변경되었으므로 생성자에서도 아래와 같이 수절해야 한다.
	 컬렉션을 초기화 한다. 카운트가 필요없으므로 매개변수num도 삭제한다.
	 */
...
//			myFriends[numOfFriends++] = high;
			/*
			 기존 인스턴스 배열에서는 인덱스를 위한 카운트가 필요했지만 컬렉션에서는 필요하지 
			 않다. 자동인덱싱이 지원되므로 add()를 통해 추가하기만 하면 된다.
			 */
			lists.add(high);
		}
		else if (choice == 2) {
...
//	} // end deleteInfo
	...
	public void searchInfo() {
		...
		for(int i = 0; i < lists.size(); i++) {
			/*i번째 인덱스의 인스턴스를 접근한 후 name을 얻어온다. 이를 통해 입력한 이름과
			 동일한지 확인한다. */
			// equals()를 통한 문자열 비교
//			if(searchName.equals(lists.get(i).name)) {
			...
			if (isFind == false) {
				System.out.println("**귀하가 요청하는 정보를 찾지 못했습니다**");
			}
		}
	...
	public void deleteInfo() {
		...
		/*
		 List컬렉션에 저장된 인스턴스를 기반으로 Iterator인스턴스를 생성한다. 이 떄
		 타입매개변수는 List와 동일하게 정의하면된다.
		 */
		Iterator<Friend> itr = lists.iterator();
		// 저장된 인스턴스의 갯수(컬렉션 크기)만큼 반복한다.
		while(itr.hasNext()) {
			...
			if(isFind == false) {
				System.out.println("**귀하가 요청한 정보가 없습니다**");
			}
		}
