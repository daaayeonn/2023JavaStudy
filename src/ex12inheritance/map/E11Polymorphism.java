...
/*
 다형성(Polymorphism)
 : 하나의 인스턴스를 여러 타입의 참조변수를 통해 참조할 때 참조변수의 타입에
 	따라 다양한 형태의 인스턴스를 이용할 수 있는 특성을 말한다.
...
class Parent {
	int parentMemner;
	void parentMethod() {
		System.out.println("부모의 메소드: patrntMethod()");
	}
}
...
class Child extends Parent {
	...
	int childMember;
	void childMethod() {
		System.out.println("자식의 메소드: childMehod()");
	}
	...
	/*
	 메서드명은 동일하지만 매개변수가 틀리므로 오버로딩으로 정의된메서드 따라서 
	 자식에서 확장한 멤버가 된다.
	 */
	void parentMethod(int childMember) {
		this.childMember = childMember;
		System.out.println("Overloading: 자식에사 확장한 메소드"
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		/*
		 이질화 : 부모타입의 변수로 자식인스턴스를 참조하는 것을 말한다. 이 경우
		 자식클래스에서 정의한 멤버는 접근할 수 없다.
		 만약 접근해야한다면 자식타입으로 강제형변환(다운캐스팅) 해야한다.
		 */
		Parent parent1 = homeChild;
		parent1.parentMemner = 1;
		// 부모타입으로 자식인스턴스에 접근할 수 없어 에러발생
//		parent1.childMethod = 1; // 에러발생
		//오버라이딩 한 메서드이므로 자식쪽이 호출된다.
		parent1.parentMethod();
		
		// 부모 참조변수로 자식 인스턴스를 참조한 두번째 i변수 생성
		Parent parent2 = new Child();
		parent2.parentMemner = 1;
		parent2.parentMethod();
		/*
		 부모 참조변수로 접근하려면 강제형변환 해야한다. 하지만 아래와같이
		 기술하면 에러가 발생한다.
		 (Child)parent2.childMember
		 소괄호를 하나 더 추가해서 parent2를 자식타입으로 형변환 후 멤버에 접근해야한다.
		 */
		...
		/*
		 자식타입으로 형변환 이후에 해당 참조변수를 통해 자식 멤버에 접근한다. 위와 동일한
		 방법이다.
		 */
		...
		/*
		 Java에서 생성한 모든 클래스는 Object클래스를 상속한다. 따라서 모든 인스턴스는
