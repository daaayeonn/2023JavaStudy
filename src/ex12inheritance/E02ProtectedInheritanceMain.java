...
/* 
 ...
 단, 부모클래스는 반드시 public으로 선언해야 한다. 만약 default로 선언하면 접근 자체가 
 불가능하므로 import도 할 수 없게 된다.
 */
...
class Seoul extends Korea {
	...
	public Seoul(String name, String city) {
		/*
		 자식클래스에서는 반드시 부모클래스의 생성자 호출을 통해 먼저 인스턴스를 생성해야한다. 그 후
		 자식 인스턴스가 메모리에 생성된다.
		 */
		super(name); // 부모생성자 호출
		this.city = city;
...
//		super.privateMethod(); // 발생
		/*
		 이와같이 직접 접근할 수 없는 메서드는 접근 가능한 멤버메서드를 통해 간접 호출하면 된다.
		 */
	}
}
