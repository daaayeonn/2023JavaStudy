...
/*
 싱글톤 디자인 패턴
 : 클래스를 설계하는 디자인패턴의 하나로써 하나의 인스턴스 즉 하나의 메모리를 할당한 후 이를 프로그램
 전체에서 공유해서 사용하고할때 쓰는 패턴이다.
 
 작성방법
 1. 생성자의 접근지정자를 private 으로 선언한다.
...
class NoSingleTone {
	int instVar;
	/*
	 생성자는 대부분 public으로 선언한다. 만약 private으로 선언하면 클래스 외부 혹은 
	 다른 패키지에서는 접근할 수 없기 때문이다.
	 */
	public NoSingleTone() {}
}
...
class SingleTone {
	...
	int shareVar;
	/*
	 정의한 클래스와 동일한 타입으로 선언된 정적 멤버변수로 JVM(자바가상머신)에 의해 프로그램이
	 시작될 때 미리 Method영역에 로드되어 사용할 준비를 마치게된다.
	 */
	private static SingleTone single = new SingleTone();
	
	/*
	 생성자를 private으로 선언하면 클래스 외부에서는 호출할 수 없으므로 new를 통해 인스턴스를
	 생성할 수 없게된다.
	 */
	private SingleTone() {
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		/*
		 일반적인 방식의 인스턴스 생성이므로, new로 생성할때마다 새로운 참조값(주소값)을 
		 할당받게된다.
		 */
		...
		/*
		 생성자가 private으로 선언되어 새로운 인스턴스를 생성할 수 없다. 생성자가 
		 "not visible" 즉 보이지 않는다는 에러가 발생한다.
		 */
//		SingleTone st1 = new SingleTone();
		
		/*
		 정적메서드이므로 클래스명을 통해 직접 호출할 수 있다. 해당 메서드 호출을 통해 싱글톤
		 인스턴스의 참조값을 얻어올 수 있다.
		 */
		...
		/*
		 결국 동일한 인스턴스를 사용한 것이므로 마지막에 할당된 값이 출력된다.
		 */
