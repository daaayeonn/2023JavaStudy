...
public class E07TypeCasting {
	...
	public static void main(String[] args) {
		...
		byte b1 = 65;
		short s1;
		//byte가 short보다 작은자료형이므로 자동형변환 된다.
		s1 = b1;
		System.out.printf("b1은 %d, s1은 %d%n", b1, s1);
		
		/*
		 아래 코드는 자동형변환이 아니다. CPU는 int보다 작은 자료를 연산할 때는
		 int로 간주하여 진행하고 결과도 int로 반환한다.
		 따라서 int형에 최적화된 CPU의 특성이라 생각하면된다.
		 */
		int num1 =  b1 + s1;
...
//		char ch1 = b1;
		...
		/*
		 명시적 형변환 : 큰 상자의 자료를 작은 상자의 잘료형에 할당하여 할 때
		 사용한다. 단, 자료의 손실이 있을 수 있으므로 필요한 경우에만 사용해야한다.
		 */
		...
		/*
		 서식문자 사용 시 %.2f는 정수부는 모두 출력하고, 소수 이하는 2자리까지만 표현한다.
		 */
		
		/*
		 문자는 메모리에 저장 시 아스키코드로 저장되므로 int과의 연산을 진행한 후 문자로
		 표현하고 char형으로 강제형변환 하면된다.
		 */
		char ch3 = 'A', ch4;
		int num4 = 2;
