...
public class Ex02PreDefineException1 {
	...
	public static void main(String[] args) {
		...
		/**
		 ...
		 예외가 발생되면 JVM(자바가상머신)은 해당 예외에 관련된 인스턴스를 만들어 프로그램쪽으로
		 전달한다. 이 때 catch절에서 예외인스턴스를 잡아 주는 것을 "예외처리"라고 한다.
		 만약 잡아주지 않는다면 프로그램은 비정상적인 종료로 이어지게 된다.
		 */
		try {
			intArr[0] = 100;
			...
			/**
			 예외가 발생되는 지점
			 해당 코드가 실행될 때 예외가 발생하게 되고 JVM아 예외객체를 생성해서
			 프로그램쪽으로 던진다(throw)
			 */
			intArr[2] = 300;
			// 이 부분은 실행되지 않는다.
		...
		/**
		 예외처리를 통해 실행의 흐름이 중단되지 않으므로 아래 문장은 정상적으로 출력된 후
		 프로그램이 종료된다.
		 */
		System.out.println("0번방 재출력: " + intArr[0]);
		System.out.println
		("### ArrayIndexOutOfBoundsException 발생 후 ###");
	} // end of main
