...
/*
 ...
 - 반환값이 없는 void는 메서드도 실행이 완료되면 호출한 위치로 실행의 흐름이 돌아온다.
 */
...
/* github 연동 완료*/
	...
	/*
	  메서드2 : 반환값X, 매개변수X 형태
	  => 전달받는 값 없이 이름을 입력받은 후 namePrint()를 호출한다.
	...
	/*
	  메서드3 : 반환값X, 매개변수O 형태
	  => 문자열을 인수로 전달받아 단순히 출력만한다. 반환값이 없는 경우에는 실행이 종료되면
	  호출했던 지점으로 실행의 흐름이 이동된다.
	 */
